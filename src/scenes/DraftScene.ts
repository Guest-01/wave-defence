...
/** 웨이브 클리어 후 3택1 드래프트 오버레이 — 순차 등장 · 호버 리프트 · 선택 연출 */
export class DraftScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const containers = this.cards.map((key, i) => {
      ...
      return this.buildCard(x, cy + 28, key);
    });
  ...
  private buildCard(x: number, y: number, key: CardKey): Phaser.GameObjects.Container {
    ...
    box.add(
      this.add.text(0, -30, def.name, { fontSize: '23px', color: '#ffffff', fontFamily: UI.FONT, fontStyle: 'bold' }).setOrigin(0.5),
    );
    box.add(
      this.add
        .text(0, 46, def.desc, {
