 ...
 * kamikaze는 자폭, attacker는 대상을 파괴할 때까지 지속 공격한다.
 */
export class Enemy {
  ...
  constructor(scene: GameScene, key: EnemyKey, x: number, y: number, hpScale: number) {
  ...
  update(dt: number, scene: GameScene): void {
    ...
    if (this.def.behavior === 'attacker') {
      ...
      if (this.target || this.attackingCore) {
        ...
        if (this.attackCooldown <= 0) {
          ...
          if (this.target) this.target.takeDamage(this.def.damage, scene);
    ...
    if (hit) {
      if (this.def.behavior === 'kamikaze') {
        hit.takeDamage(this.def.damage, scene);
        this.die(scene, false);
      } else {
        this.target = hit;
        this.attackCooldown = this.def.attackInterval ?? 1; // 첫 타격 전 준비 시간
  ...
  private die(scene: GameScene, killed: boolean): void {
    this.hp = 0;
    this.body.destroy();
    this.hpBg.destroy();
    this.hpFill.destroy();
    scene.onEnemyDead(this, killed);
  }
}
